...
fn solve() {
    ...
    let end_mins = vec![mins.get(&(x,y,0 as usize)).unwrap_or(&69696969),
...
fn traverse(grid: &Vec<Vec<String>>, mins: &mut HashMap<(usize,usize,usize), i32>, cur_dir: usize, cx: usize, cy: usize) {
    let dirs: Vec<(isize, isize)> = vec![(0,1),(1,0),(0,-1),(-1,0)];
...
fn is_reasonable_diff(next: (usize, usize, usize), cur: (usize, usize, usize), mins: &HashMap<(usize,usize,usize), i32>) -> Vec<usize> {
    let mut is_reasonable: Vec<usize> = vec![];
    for d in 0..4 {
        let diff = *mins.get(&cur).unwrap_or(&-99000000) - *mins.get(&(next.0, next.1, d)).unwrap_or(&-1000000);
        if diff == 1 || diff == 1001 {
